...
btnAudioSunFlowers.addEventListener('click', () => {
  // Pausa el otro audio si está reproduciéndose
  if (!audioSunFlowers.paused) return;
  audioBolanos.pause();
  audioSunFlowers.play();
});

// Función para manejar el clic en el botón de Bolanos
btnAudioBolanos.addEventListener('click', () => {
  // Pausa el otro audio si está reproduciéndose
  if (!audioBolanos.paused) return;
  audioSunFlowers.pause();
  audioBolanos.play();
});

// DOM selectors
const containerEl = document.querySelector('.content-animation');
...
function init() {
    ...
    // const flowerTexture = new THREE.TextureLoader().load('./public/flowers.png');
...
function updateCursorOpacity() {
    ...
    // } else {
    //     cursorMesh.material.opacity = 1;
    // }
}


// Implementación de emoji confetti
